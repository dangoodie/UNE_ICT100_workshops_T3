...
let subFaceRecSetup = manager.subscribe("webcam_ready", async (msg) => {
...
let subHandleFaceDetectionResults = manager.subscribe(
  ...
  (msg) => {
    ...
    if (detections.length > 0) {
      manager.publish("user_engagment", {});
    }
    detectFace();
  }
);
...
async function enrollFace(botName) {
    ...
    do {
        ...
        if (success) {
            ...
            if (enrolled) {
                await speak(botName, `I have memorised your identity.`);
            } else {
                await speak(botName, `I'm sorry, I couldn't memorise your identity.`);
            }
        } else {
            name = "";
            await speak(botName, `I'm sorry, I didn't get your name. What's your name?`);
        }
...
let subHandleEngagement = manager.subscribe("user_engagment", async (msg) => {
    if (!isEngaging) {
        isEngaging = true;
        let botName = "pico";
